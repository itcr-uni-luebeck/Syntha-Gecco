...
with open("maps/codex-term-code-mapping.json") as mapping_file:
...
def generate_unit_test(bundle, output_path):
    ...
    for entry in bundle["entry"]:
        ...
        if resource_type == "Condition":
            ...
            # write_sq_to_file(sq_age, resource, test_cases_dir, "1-age")
            sq_ethnic_group = generate_ethnic_group_sq(resource)
            sq_ethnic_group.version = "http://to_be_decided.com/draft-1/schema#"
            # write_sq_to_file(sq_ethnic_group, resource, test_cases_dir, "1-ethnic_group")
            continue
        ...
        write_sq_to_file(sq, resource, output_path)


def write_sq_to_file(structured_query, resource, test_cases_dir, file_name=None):
    if file_name:
        file = open(os.path.join(test_cases_dir, file_name + ".json"), 'w')
    else:
        file = open(os.path.join(test_cases_dir, resource["meta"]["profile"][0].split("/")[-1] + "-" +
